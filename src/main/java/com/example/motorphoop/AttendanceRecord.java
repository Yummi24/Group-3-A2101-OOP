...
public class AttendanceRecord {
    ...
    public static double[] getAttendanceForEmployee(String employeeID, String monthYearKey) {
        if (employeeAttendanceData.containsKey(employeeID)) {
            String[] data = employeeAttendanceData.get(employeeID);
            double daysWorked = Double.parseDouble(data[0]);
            double hoursWorked = Double.parseDouble(data[1]);
            return new double[]{daysWorked, hoursWorked};
        }
        return new double[]{0, 0}; // Return zero if no data is found
    }
